...
public class ExporterEntry
{
    ...
    // key: WebaCustomMaterial componenet or UnityMateiral InistanceID
    ...
    public NodeId SaveNode(Transform tr, bool savingColliders = false)
    {
        ...
        //configure Webaverse collider schema
        JProperty extras;
        ...
        if (colliderParent != null)
        {
            extras = new JProperty("extras", new JObject(
                new JProperty("Webaverse.collider.bodyType", 0),
                new JProperty("Webaverse.entity", tr.name)
            ));
        }
        else
        {
    ...
    public MaterialId SaveComponentMaterial(WebaCustomMaterial material)
    {
        if (root.Materials == null)
        {
            root.Materials = new List<GLTF.Schema.Material>();
    ...
    public CubeTextureId SaveCubeTexture(Cubemap texture, bool hasTransparency = false, string path = null, int maxSize = -1, EHDRTextureType hdrType = EHDRTextureType.DEFAULT)
    {
        ...
        var extName = ExtensionManager.GetExtensionName(typeof(WebaCubeTextureExtensionFactory));
        /*ExtensionManager.Serialize(ExtensionManager.GetExtensionName(typeof(WebaCubeTextureExtensionFactory)), this, root.Extensions, texture, new CubeTextureSaveOptions{
            maxSize = maxSize, sampler = samplerId, hasTransparency = hasTransparency, path = path, hdrType = hdrType
        });*/

        var id = new CubeTextureId { Id = ((WebaCubeTextureExtension)root.Extensions[extName]).textures.Count - 1, Root = root };
    ...
    public CubeTextureId SaveCubeTexture(Texture2D[] textures, bool hasTransparency = false, string path = null, int maxSize = -1, EHDRTextureType hdrType = EHDRTextureType.DEFAULT)
    {
        ...
        var extName = ExtensionManager.GetExtensionName(typeof(WebaCubeTextureExtensionFactory));
        /*ExtensionManager.Serialize(ExtensionManager.GetExtensionName(typeof(WebaCubeTextureExtensionFactory)), this, root.Extensions, null, new CubeTextureSaveOptions{
    ...
    public ImageId SaveImage(Texture2D texture, bool hasTransparency = false, string path = null, int maxSize = -1, EHDRTextureType hdrType = EHDRTextureType.DEFAULT, bool flipY = true)
    {
        ...
        if (isHDR)
        {
            ...
            {
                Utils.ThrowException("HDR Texture can only be exported as 'RGBD' now !");
            }
        ...
        Utils.DoActionForTexture(ref newTex, tex =>
        {
            ...
            {
                content = tex.EncodeToJPG(ExporterSettings.NormalTexture.jpgQuality);
            }
        });
    ...
    private TextureId GenerateTexture(Texture2D texture, ImageId imageId)
    {
        ...
        //ExtensionManager.Serialize(ExtensionManager.GetExtensionName(typeof(WebaTextureImproveExtensionFactory)), this, gltfTexture.Extensions, texture);
    ...
    private GLTF.Schema.Camera ProcessCamera(UnityEngine.Camera camera)
    {
        ...
        if (ExporterSettings.Export.skybox) {
            ...
            //ExtensionManager.Serialize(ExtensionManager.GetExtensionName(typeof(WebaSkyboxExtensionFactory)), this, cam.Extensions, camera);
        }
    ...
    public SkinId SaveSkin(Transform tr)
    {
        ...
        foreach (var bone in skinMesh.bones)
        {
            ...
            {
                Utils.ThrowException("You are expoting a skinned mesh '" + node.Name + "', but not select bones!");
            }
    ...
    public void SaveAnimations(Transform tr)
    {
        ...
        {
            Utils.ThrowException("Never support Unity.Animation component now, use Unity.Animator to instead of it !");
        }
        ...
        WebaAnimator animator = tr.GetComponent<WebaAnimator>();
        if (animator == null)
        {
            animator = tr.gameObject.AddComponent<WebaAnimator>();
        }
    ...
    private List<string> BakeAnimationClip(GLTF.Schema.Animation anim, Transform tr, AnimationClip clip)
    {
        ...
        foreach (var binding in AnimationUtility.GetCurveBindings(clip))
        {
            ...
            {
                Utils.ThrowException(new Exception(
