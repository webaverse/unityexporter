...
public class StandardToWebaPBR : MonoBehaviour
{
    static Dictionary<Material, string> backups;
    static Dictionary<Material, Material> gltfLinks;

    [MenuItem("Assets/Materials/Materials to WebaPBR", priority = 0)]
    public static void AssetsToWebaPBR()
    {
        backups = new Dictionary<Material, string>();
        var objects = Selection.objects;
        AssetsToWebaPBR(objects);

        AssetDatabase.Refresh();
    }

    private static void AssetsToWebaPBR(UnityEngine.Object[] objects)
    {
        ...
        foreach (var obj in objects)
        {
            ...
            {
                ToWebaPBR(obj as Material, needBak);
            }
    ...
    [MenuItem("CONTEXT/Material/To WebaPBR", priority = 0)]
    private static void InspectorToWebaPBR(MenuCommand command)
    {
    ...
    [MenuItem("GameObject/Weba/Materials to WebaPBR", priority = 11)]
    private static void GOToWebaPBR()
    {
        ...
        foreach (var m in materials)
        {
            ToWebaPBR(m.Item2, needBak, m.Item1);
        }
    ...
    [MenuItem("Assets/Materials/Materials to WebaPBR", priority = 4)]
    public static void AllToWebaPBR()
    {
        backups = new Dictionary<Material, string>();
    ...
    private static void ToWebaPBR(Material material, bool backup = true, Renderer renderer = null)
    {
    ...
    {
        var material = new Material(Shader.Find("Weba/PBR"));

        bool isMetal = mat.shader == glShader || mat.shader.name == "Standard" || mat.shader.name == "Autodesk Interactive";
        if (!isMetal)
        {
            material.SetInt("workflow", (int)WebaPBRShaderGUI.Workflow.Specular);
        }
        else
        {
            material.SetInt("workflow", (int)WebaPBRShaderGUI.Workflow.Metal);
        }

        var mode = GetBlendMode(mat);
        material.SetInt("_Mode", (int)mode);
        if (mode == WebaPBRShaderGUI.BlendMode.Cutout)
        {
        ...
        */
        ///*
        mat.shader = Shader.Find("Weba/PBR");
    ...
    private static WebaPBRShaderGUI.BlendMode GetBlendMode(Material mat)
    {
        if (!mat.HasProperty("_Mode"))
        {
            return WebaPBRShaderGUI.BlendMode.Opaque;
        }
        ...
        switch ((int)mat.GetFloat("_Mode"))
        {
            ...
            // Cutout
            case 1:
                return WebaPBRShaderGUI.BlendMode.Cutout;
            // Transparent
            case 2:
            case 3:
                return WebaPBRShaderGUI.BlendMode.Transparent;
        }
