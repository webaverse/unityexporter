...
public class ExporterUtils
{
    ...
    )
    {
        ...
        {
            Utils.ThrowException("Only support packing float, int, Vector2, Vector3, Vector4, BoneWeight, Matrix4 and Color now !");
        }
    ...
    public static GLTF.Schema.Material ConvertMaterial(UnityEngine.Material material, ExporterEntry entry, Renderer renderer = null)
    {
        ...
        {
            Utils.ThrowException("Toolkit doesn't support Unity Standard Material anymore, please use converter to convert them to 'Weba/PBR'");
        }

        if (material.shader.name == "Weba/PBR")
        {
            return ConvertWebaPBRMaterial(material, entry, renderer);
        }

        if (material.shader.name.Contains("Weba/"))
        {
            return ConvertWebaCustomMaterial(material, entry);
        }

        //return ConvertKHRWebGLMaterial(material, entry);
        Utils.ThrowException("Only support Weba/PBR or Weba/XXX(CustomMaterial) now !");
        return null;
    }
    ...
    private static Texture2D GenerateBakedDiffuse(Texture2D tex, Texture2D lightmapTex, UnityEngine.Mesh mesh, Renderer renderer)
    {
        ...
        ComputeShader combiner = AssetDatabase.LoadAssetAtPath<ComputeShader>(@"Assets/Webaverse/Code/Shaders/LightmapCombineBaker.compute");
    ...
    private static GLTF.Schema.Material ConvertWebaPBRMaterial(UnityEngine.Material mat, ExporterEntry entry, Renderer renderer)
    {
        ...
        if (mat.GetInt("envReflection") != (int)WebaPBRShaderGUI.EnvReflection.Off || (ExporterSettings.Lighting.ambient && (RenderSettings.ambientMode == UnityEngine.Rendering.AmbientMode.Skybox || RenderSettings.ambientMode == UnityEngine.Rendering.AmbientMode.Trilight)))
        {
            if (material.Extensions == null)
            {
                material.Extensions = new Dictionary<string, Extension>();
            }
            //ExtensionManager.Serialize(ExtensionManager.GetExtensionName(typeof(WebaImageBasedLightingExtensionFactory)), entry, material.Extensions, mat);
        }

        return material;
    }

    public static GLTF.Schema.Material ConvertWebaCustomMaterial(UnityEngine.Material mat, ExporterEntry entry)
    {
        if (_tempGO == null)
        {
        ...
        var customMaterial = _tempGO.AddComponent<WebaCustomMaterial>();
        var className = mat.shader.name.Replace("Weba/", "");
        ...
        var floatArray = new List<WebaMaterialUniformFloat>();
        var vector4Array = new List<WebaMaterialUniformFloatVec4>();
        var colorArray = new List<WebaMaterialUniformColor>();
        var textureArray = new List<WebaMaterialUniformTexture>();
        ...
        for (int i = 0; i < ShaderUtil.GetPropertyCount(mat.shader); i += 1)
        {
            ...
            switch (propType)
            {
                ...
                case ShaderUtil.ShaderPropertyType.Range:
                    floatArray.Add(new WebaMaterialUniformFloat { name = propName, value = mat.GetFloat(n) });
                    break;
                case ShaderUtil.ShaderPropertyType.Color:
                    colorArray.Add(new WebaMaterialUniformColor { name = propName, value = mat.GetColor(n) });
                    break;
                case ShaderUtil.ShaderPropertyType.Vector:
                    vector4Array.Add(new WebaMaterialUniformFloatVec4 { name = propName, value = mat.GetVector(n) });
                    break;
                case ShaderUtil.ShaderPropertyType.TexEnv:
                    if (mat.GetTexture(n) != null)
                    {
                        textureArray.Add(new WebaMaterialUniformTexture { name = propName, value = (Texture2D)mat.GetTexture(n) });
                    }
                    break;
            }
    ...
    public static GLTF.Schema.Material ConvertMaterial(WebaCustomMaterial mat, ExporterEntry entry)
    {
        var material = new GLTF.Schema.Material();
        ...
        //ExtensionManager.Serialize(ExtensionManager.GetExtensionName(typeof(WebaCustomMaterialExtensionFactory)), entry, material.Extensions, mat);
        return material;
    }
