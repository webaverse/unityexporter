...
public class EditorExporter
{
    ...
    private void ExportOne(ExporterEntry entry)
    {
        ...
        //ExtensionManager.Serialize(ExtensionManager.GetExtensionName(typeof(WebaRendererExtensionFactory)), entry, entry.root.Extensions);

        

        // if ambientMode is not Flat, use WebaImageBaseLighting extension
        if (ExporterSettings.Lighting.ambient && RenderSettings.ambientMode == UnityEngine.Rendering.AmbientMode.Flat)
        {
            //ExtensionManager.Serialize(ExtensionManager.GetExtensionName(typeof(WebaAmbientLightExtensionFactory)), entry, entry.root.Extensions);
        }
    ...
    private void ExportNode(Transform tr, ExporterEntry entry)
    {
        ...
        var WebaNode = tr.GetComponent<WebaNode>();
        if (WebaNode)
        {
            if (WebaNode.selfType == EWebaNodeType.Actor)
            {
            ...
            else
            {
                ...
                while (parent)
                {
                    var parentWebaNode = parent.GetComponent<WebaNode>();
                    if (parentWebaNode == null || parentWebaNode.selfType == EWebaNodeType.Component)
                    {
    ...
    private void ExportMesh(Transform tr, ExporterEntry entry)
    {
        ...
        if (tr.GetComponent<WebaSprite>() != null)
        {
            return;
        }
        ...
        if (renderer.enabled && renderer.gameObject.activeInHierarchy)
        {
            ...
            {
                var materialComponents = tr.GetComponents<WebaCustomMaterial>();
                int i = 0;
    ...
    private void ExportComponentMaterial(WebaCustomMaterial material, MeshPrimitive primitive, ExporterEntry entry)
    {
    ...
    private bool CheckSkinValidity(SkinnedMeshRenderer skin, ExporterEntry entry)
    {
        foreach (Transform tr in skin.bones)
        {
            ...
            {
                Utils.ThrowException("Error while exporting skin for " + skin.name + " (skipping skinning export).\nClick for more details:\n \nThe following bones are used but are not selected" + tr.name + "\n");
                return false;
            }
        }
