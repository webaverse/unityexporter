...
public class ExtensionManager
{
    ...
    public static Dictionary<string, WebaExtensionFactory> Name2Extensions = new Dictionary<string, WebaExtensionFactory>();
    ...
    public static void Init()
    {
        ...
        foreach (var clazz in Assembly.GetAssembly(typeof(WebaExtensionFactory)).GetTypes())
        {
            if (clazz.IsClass && !clazz.IsAbstract && clazz.IsSubclassOf(typeof(WebaExtensionFactory)))
            {
                if (clazz == typeof(WebaRendererExtensionFactory)) continue;
                if (clazz == typeof(WebaNodeExtensionFactory)) continue;
                Register(clazz);
            }
        }
    }
    ...
    {
        var factory = (WebaExtensionFactory)Activator.CreateInstance(FactoryClass);
